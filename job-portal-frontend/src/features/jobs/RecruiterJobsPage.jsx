  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-white mb-1.5">Jobs</h1>
          <p className="text-gray-400 text-sm">Jobs you've posted</p>
        </div>
        ...
        <div className="space-y-3">
          {jobs.map((job) => (
            <Link
              ...
              className="flex items-center justify-between gap-4 rounded-xl border border-white/10 bg-white/[0.03] hover:bg-white/[0.05] px-5 py-4 transition-all"
            >
              <div className="min-w-0">
                <p className="text-white font-medium truncate">{job.title}</p>
                <p className="text-xs text-gray-500 mt-1">{job.company_name} · {job.location}</p>
              </div>
