...
const selectClass =
  'w-full px-4 py-2.5 rounded-xl bg-black/20 border border-white/10 text-white text-sm focus:outline-none focus:ring-[3px] focus:ring-primary/30 focus:border-primary/60';
...
export default function NewJobPage() {
  ...
  return (
    <div className="max-w-xl">
      ...
      <h1 className="text-2xl font-bold text-white mb-1.5">Post a Job</h1>
      <p className="text-gray-400 text-sm mb-8">Fill in the details to publish a new opening</p>

      <form onSubmit={handleSubmit} className="rounded-2xl border border-white/10 bg-white/[0.03] p-6 sm:p-8">
        <div className="mb-4">
          <label className="block text-[13px] font-medium text-gray-300 mb-1.5">Company</label>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          <div>
            <label className="block text-[13px] font-medium text-gray-300 mb-1.5">Job Type</label>
            <select className={selectClass} value={form.job_type} onChange={(e) => setForm({ ...form, job_type: e.target.value })}>
              {JOB_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-[13px] font-medium text-gray-300 mb-1.5">Work Location</label>
