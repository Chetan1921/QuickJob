...
export default function ProfilePage() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10">
      ...
      {/* Header card */}
      <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6 sm:p-8 mb-6">
        <div className="flex flex-col sm:flex-row sm:items-center gap-5">
          <div className="relative shrink-0">
            <div className="h-20 w-20 rounded-2xl bg-primary/15 border border-primary/30 overflow-hidden flex items-center justify-center text-2xl font-bold text-indigo-300">
            ...
            <button
              ...
              className="absolute -bottom-1.5 -right-1.5 h-7 w-7 rounded-full bg-primary border-2 border-dark-bg flex items-center justify-center text-white hover:brightness-110"
            >
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6 sm:p-8 mb-6">
        ...
        {editing ? (
          ...
          <div className="space-y-3 text-sm">
            <div className="flex justify-between"><span className="text-gray-500">Phone</span><span className="text-gray-200">{profile?.phone_number || '—'}</span></div>
            <div className="flex justify-between gap-4"><span className="text-gray-500 shrink-0">Bio</span><span className="text-gray-200 text-right">{profile?.bio || '—'}</span></div>
          </div>
        )}
      </div>
      ...
      {user?.role === 'job_seeker' && (
        <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6 sm:p-8 mb-6">
          ...
          <div className="flex items-center justify-between gap-4 px-4 py-3 rounded-xl bg-black/20 border border-white/10">
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6 sm:p-8">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {skillsList.length === 0 && <p className="text-gray-500 text-sm">No skills added yet.</p>}
          {skillsList.map((skill) => (
            <span key={skill} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium bg-primary/10 text-indigo-300 border border-primary/20">
              {skill}
              <button onClick={() => handleDeleteSkill(skill)} className="hover:text-white">
                <X className="h-3 w-3" />
              </button>
            </span>
          ))}
        ...
        <form onSubmit={handleAddSkill} className="flex gap-2">
          <input
            ...
            className="flex-1 px-4 py-2.5 rounded-xl bg-black/20 border border-white/10 text-white text-sm placeholder-gray-500 focus:outline-none focus:ring-[3px] focus:ring-primary/30 focus:border-primary/60"
          />
